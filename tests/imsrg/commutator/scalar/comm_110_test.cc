...
TEST_CASE("Test emax=4 NAT O16 [1, 1] -> 0 commutator (anti-herm, herm).") {
  ...
  SECTION("[h1, gen1]") {
    ...
    auto exp_result_0b = result_me1j.Get0BPart();
...
TEST_CASE("Test emax=6 NAT O16 [1, 1] -> 0 commutator (anti-herm, herm).") {
  using imsrg::Hermiticity;
  using imsrg::HOEnergy;
  using imsrg::ME1JFile;
