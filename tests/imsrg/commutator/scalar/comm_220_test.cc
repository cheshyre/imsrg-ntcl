...
TEST_CASE("Test emax=4 NAT O16 [2, 2] -> 0 commutator (anti-herm, herm).") {
  ...
  SECTION("[gen2, h2]") {
    ...
    const auto expected = result_me1j.Get0BPart();

    const auto result = imsrg::EvaluateScalar220Commutator(gen2, h2);
    REQUIRE(result == Approx(expected).margin(1e-5));
  }
  ...
  SECTION("[h2, gen2]") {
    ...
    const auto result = imsrg::EvaluateScalar220Commutator(h2, gen2);
    REQUIRE(result == Approx(expected).margin(1e-5));
  }
}

TEST_CASE("Test emax=6 NAT O16 [2, 2] -> 0 commutator (anti-herm, herm).",
          "[!hide][big]") {
  using imsrg::Hermiticity;
  using imsrg::HOEnergy;
  using imsrg::ME1JFile;
